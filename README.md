...
- **模型全可配置**：双 provider（Anthropic Messages / OpenAI Responses）· 角色化（parse/synthesis/deep_report/judge）+ 三级覆盖 · 自定义 API 地址（中转/代理）
...
2. 选供应商：`[api] provider`＝`anthropic`（Messages API，claude-*）或 `openai`（Responses API `/v1/responses`，gpt-*）。缺省 anthropic；切 openai 时把 `[models].*` 换成 gpt 模型名。
3. 设置 API Key（不写进配置文件）：按 provider 设环境变量 `ANTHROPIC_API_KEY` / `OPENAI_API_KEY`（fish：`set -x ...`，bash：`export ...`）；或 `uv pip install -e ".[secure]"` 后 `bellwether config set-key`（按 provider 存入钥匙串，环境变量始终优先）
4. （可选）自定义 API 地址（中转/代理）：`config.toml` 的 `[api] base_url`，或环境变量 `ANTHROPIC_BASE_URL` / `OPENAI_BASE_URL`（前者优先）
...
角色化（`parse` / `synthesis` / `deep_report` / `judge`）+ 三级覆盖：`CLI 参数 > config.toml > 内置默认`。代码任何地方都不硬编码模型名，一律经 `ModelRouter` 解析。供应商经 `[api] provider` 切换（`anthropic`＝Messages API `/v1/messages`，claude-*；`openai`＝Responses API `/v1/responses`，gpt-*），客户端由 `build_llm_client` 按 provider 工厂化、上层零改动。用第三方中转时先 `bellwether models` 查它实际支持的模型 id 再填。`judge`（评测评审员）不参与降级链——评审模型漂移会破坏评测可比性，失败即标 unverifiable（不可核验、排除出统计，非质量 fail）。
...
| A股 CN | 东财 `stock_zh_a_hist` → 新浪 `stock_zh_a_daily`（降级链） | `stock_a_indicator_lg`（akshare 已移除，估值暂空 · M3 A9 修） | 东财 `stock_news_em` |
| 港股 HK | 新浪 `stock_hk_daily` | （暂留空 · M3 A9 补） | 东财 `stock_news_em` |
...
- **LLM**：限流/瞬态退避重试；模型持续不可用时自动降一档（`deep_report`→`synthesis`→`parse`，只换模型 id 不换任务参数），报告尾部**明示**降级；认证失败立即明示不掩盖。用户显式 `--model` 时不降级。prompt caching 默认启用（system+tools 缓存断点，多轮 tool-use 大幅省 input token；中转不支持时 `[api] prompt_caching = false` 关闭；openai 端自动缓存、此开关无效），cache 读写 tokens 在成本摘要单列如实计价。
...
pytest        # 317 个单测：确定性计算全覆盖 + 数据源/LLM 两侧故障注入 + IR 构造性核验 + 评测器守门员 + OpenAI 适配器翻译
```
