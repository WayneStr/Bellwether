...
def test_input_hash_deterministic_and_sensitive():
    a1 = new_trace("AAPL", True, ["m1", "m2"], "pv")
    a2 = new_trace("AAPL", True, ["m1", "m2"], "pv")
    b = new_trace("AAPL", False, ["m1", "m2"], "pv")
...
def test_write_trace_persists_and_redacts(tmp_path, monkeypatch):
    monkeypatch.delenv("ANTHROPIC_API_KEY", raising=False)
    trace = new_trace("AAPL", False, ["m1"], "pv")
...
def test_write_trace_failure_is_silent(tmp_path):
...
def orch(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda name, tool_input, provider: '{"ok": true}',
    )
...
def test_trace_records_full_analysis(orch):
...
def test_trace_written_on_total_failure(orch):
    from bellwether.agent.llm import ResilientLLM
